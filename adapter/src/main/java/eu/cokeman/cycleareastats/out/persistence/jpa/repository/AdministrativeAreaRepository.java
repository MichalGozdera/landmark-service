...
public class AdministrativeAreaRepository
    implements eu.cokeman.cycleareastats.port.out.persistence.AdministrativeAreaRepository {
  ...
  private final JpaAdministrativeAreaSimpleRepositorySpringDataRepository
      springSimpleDataRepository;

  public AdministrativeAreaRepository(
      JpaAdministrativeAreaRepositorySpringDataRepository springDataRepository,
      JpaAdministrativeAreaSimpleRepositorySpringDataRepository
          simpleRepositorySpringDataRepository) {
  ...
  @Override
  public AdministrativeArea findSimpleByAdministrativeAreaId(
    ...
    AdministrativeArea administrativeArea =
        mapper.mapSimpleJpaToInternal(jpaArea.orElseThrow()).build();
    return administrativeArea;
  }
  ...
  public AdministrativeArea updateAdministrativeArea(
    ...
    var jpaArea =
        springDataRepository.findById(areaId.value()).orElseThrow(EntityNotFoundException::new);
  ...
  public List<AdministrativeArea> findByLevelAndCountry(String levelName, String countryName) {
    var entities =
        springDataRepository.findByLevel_NameAndLevel_Country_Name(levelName, countryName);
  ...
  @Override
  public List<AdministrativeArea> findSimpleByLevelAndCountry(
      String levelName, String countryName) {
    var entities =
        springSimpleDataRepository.findByLevel_NameAndLevel_Country_Name(levelName, countryName);
  ...
  public AdministrativeAreaId findParent(AdministrativeAreaId child) {
    try {
      ...
      log.error(
          "Error while finding parent for child area id {}: {}. Returning null!",
          child.value(),
          ex.getMessage(),
          ex);
      return null;
  ...
  public List<AdministrativeArea> findChildren(AdministrativeAreaId parent) {
    return springDataRepository.findChildren(parent.value()).stream()
        .map(e -> mapper.mapJpaToInternal(e).build())
        .toList();
  }
}
