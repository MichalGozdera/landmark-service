...
@Repository
public class AdministrativeLevelRepository
    implements eu.cokeman.cycleareastats.port.out.persistence.AdministrativeLevelRepository {
  ...
  public AdministrativeLevelRepository(
      JpaAdministrativeLevelRepositorySpringDataRepository springDataRepository,
      JpaCountryRepositorySpringDataRepository countrySpringDataRepository) {
    this.levelSpringDataRepository = springDataRepository;
  }

  @Override
  public AdministrativeLevel findByAdministrativeLevelId(
      AdministrativeLevelId administrativeLevelId) {
    var entity = levelSpringDataRepository.findById(administrativeLevelId.value());
    return mapper.mapJpaToInternal(entity.orElseThrow()).build();
  }
  ...
  public AdministrativeLevel updateLevel(AdministrativeLevelId levelId, AdministrativeLevel level) {
    ...
    var jpaLevel =
        levelSpringDataRepository
            .findById(levelId.value())
  ...
  public Optional<AdministrativeLevel> findByCountryAndName(Country country, LevelName name) {
    var foundEntity =
        levelSpringDataRepository.findByCountry_NameAndName(country.getName(), name.name());
