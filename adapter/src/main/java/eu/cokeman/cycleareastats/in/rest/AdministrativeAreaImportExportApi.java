...
@RestController
public class AdministrativeAreaImportExportApi
    implements eu.cokeman.cycleareastats.openapi.api.AdministrativeAreaImportExportApi {
  ...
  public AdministrativeAreaImportExportApi(
      ImportAdministrativeAreaUseCase importAdministrativeAreaUseCase,
      ExportAdministrativeAreaUseCase exportAdministrativeAreaUseCase) {
  ...
  @Override
  public ResponseEntity<Resource> exportAdministrativeAreasKml(
      String countryName, String levelName, String fileName) {
    String kml = exportAdministrativeAreaUseCase.exportKmlByCountryAndLevel(countryName, levelName);
    Resource resource =
        new org.springframework.core.io.ByteArrayResource(
            kml.getBytes(java.nio.charset.StandardCharsets.UTF_8));
  ...
  @Override
  public ResponseEntity<Void> importAdministrativeAreas(
      AdministrativeAreasImportRequestDto requestDto, MultipartFile geometry) {
    ...
    if (filename == null || !filename.toLowerCase().endsWith(".kml")) {
      return ResponseEntity.badRequest()
          .header("X-Error-Message", "File must have .kml extension")
          .build();
    }
