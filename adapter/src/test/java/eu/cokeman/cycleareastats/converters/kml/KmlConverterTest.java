...
class KmlConverterTest {
    ...
    void testConvertToKml() throws Exception {
        ...
        // MultiPolygon
        Polygon polyA = gf.createPolygon(gf.createLinearRing(new Coordinate[]{
        ...
        Polygon polyB = gf.createPolygon(gf.createLinearRing(new Coordinate[]{
        ...
        org.locationtech.jts.geom.MultiPolygon multiPolygon = gf.createMultiPolygon(new Polygon[]{polyA, polyB});
        ...
        assertNotNull(is, "expected-multi.kml file not found in test/resources. Generate it based on the geometries above.");
        String expectedKml = new String(is.readAllBytes(), StandardCharsets.UTF_8).replaceAll("\\s+", "");
    ...
    void testConvertToLandmarksGeometries_Lines() throws Exception {
        java.io.InputStream is = getClass().getClassLoader().getResourceAsStream("kml-cases/test-lines.kml");
        assertNotNull(is, "test-lines.kml file not found in resources");
        byte[] kmlBytes = is.readAllBytes();
        MultipartFile mockFile = Mockito.mock(MultipartFile.class);
        Mockito.when(mockFile.getResource()).thenReturn(new ByteArrayResource(kmlBytes) {
            @Override
        ...
        for (AdministrativeAreaGeometry geom : result) {
            if ("TestArea1".equals(geom.name())) {
                assertEquals("MultiPolygon", ((org.locationtech.jts.geom.Geometry)geom.geometryData()).getGeometryType());
                found1 = true;
            } else if ("TestArea2".equals(geom.name())) {
                assertTrue(((org.locationtech.jts.geom.Geometry)geom.geometryData()).getGeometryType().contains("MultiPolygon") ||
                           ((org.locationtech.jts.geom.Geometry)geom.geometryData()).getGeometryType().contains("GeometryCollection"));
                found2 = true;
    ...
    void testSimplePolygon() throws Exception {
        java.io.InputStream is = getClass().getClassLoader().getResourceAsStream("kml-cases/simple-polygon.kml");
    ...
    void testPolygonsWithHoles() throws Exception {
        java.io.InputStream is = getClass().getClassLoader().getResourceAsStream("kml-cases/polygons-with-holes.kml");
    ...
    void testInvalidGeometry() throws Exception {
        java.io.InputStream is = getClass().getClassLoader().getResourceAsStream("kml-cases/invalid-geometry.kml");
    ...
    void testOnlyMultiGeometry() throws Exception {
        java.io.InputStream is = getClass().getClassLoader().getResourceAsStream("kml-cases/only-multigeometry.kml");
        ...
        for (AdministrativeAreaGeometry geom : result) {
            assertTrue(geom.geometryData() instanceof org.locationtech.jts.geom.MultiPolygon, "Should always be MultiPolygon");
        }
    }

    // Helper method for loading and parsing KML from test resources
    private Set<AdministrativeAreaGeometry> parseKmlFromResource(String resourcePath) throws Exception {
    ...
    void testTestLines() throws Exception {
        // Should parse three MultiPolygon geometries from test-lines.kml (all lines are converted to MultiPolygon)
    ...
    void testMissingMultiGeometryPlacemark() throws Exception {
        ...
        Set<AdministrativeAreaGeometry> result = parseKmlFromResource("kml-cases/mixed-multigeometry.kml");
