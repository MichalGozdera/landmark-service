...
class AdministrativeAreaRepositoryTest {
  ...
  @Autowired private AdministrativeAreaRepository repository;

  @Autowired private JpaCountryRepositorySpringDataRepository countryRepository;

  @Autowired private JpaAdministrativeLevelRepositorySpringDataRepository levelRepository;
  ...
  void testFindByMetadataContaining() throws Exception {
    var area =
        prepareAreaEntity(
            "parent-area.json", "Fragment Test Area", null, "{\"desc\":\"fragment description\"}");
    var domainArea = AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(area).build();
    repository.createArea(domainArea);
    var result = repository.findByMetadataContains("fragment");
    assertThat(result.stream().map(a -> a.getName().name()).toList())
        .contains("Fragment Test Area");
  }
  ...
  void testFindParentIdWithoutExplicitParentRelation() throws Exception {
    ...
    var parentId =
        repository.createArea(
            AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(parentArea).build());
    var childId =
        repository.createArea(
            AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(childArea).build());
  ...
  void testFindChildrenOnlyInsideParentGeometry() throws Exception {
    ...
    var childOutside =
        prepareAreaEntity("child-area-outside.json", "Child Outside", childLevel, null);
    var parentId =
        repository.createArea(
            AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(parentArea).build());
    repository.createArea(
        AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(childInside).build());
    repository.createArea(
        AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(childOutside).build());
    var children = repository.findChildren(parentId);
  ...
  void testFindParentIdMultipleParentsReturnsNull() throws Exception {
    ...
    var childArea = prepareAreaEntity("child-area.json", "Child Area", childLevel, null);
    repository.createArea(
        AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(parentArea1).build());
    repository.createArea(
        AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(parentArea2).build());
    var childId =
        repository.createArea(
            AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(childArea).build());
  ...
  void testDeleteAreaRemovesArea() throws Exception {
    ...
    org.assertj.core.api.Assertions.assertThatThrownBy(
            () -> repository.findByAdministrativeAreaId(id))
  ...
  void testFindSimpleByIdThrowsForNonExisting() {
    org.assertj.core.api.Assertions.assertThatThrownBy(
            () -> repository.findSimpleByAdministrativeAreaId(new AdministrativeAreaId(-999)))
        .isInstanceOf(Exception.class);
  }
  ...
  void testFindSimpleByMetadataContainsReturnsSimpleList() throws Exception {
    var area =
        prepareAreaEntity(
            "parent-area.json", "Simple Metadata Area", null, "{\"desc\":\"meta-fragment\"}");
    var domainArea = AdministrativeAreaJpaMapper.INSTANCE.mapJpaToInternal(area).build();
    repository.createArea(domainArea);
    var result = repository.findSimpleByMetadataContains("meta-fragment");
    assertThat(result.stream().map(a -> a.getName().name()).toList())
        .contains("Simple Metadata Area");
    assertThat(result.stream().allMatch(a -> a.getGeometry() == null)).isTrue();
  }
  ...
  private AdministrativeAreaEntity prepareAreaEntity(
      String jsonFile, String name, AdministrativeLevelEntity level, String metadata)
      throws Exception {
