...
public class ImportAdministrativeAreaService implements ImportAdministrativeAreaUseCase {
  ...
  public ImportAdministrativeAreaService(
      AdministrativeAreaConverter converter, AdministrativeAreaDomainService domainService) {
    this.converter = converter;
    this.domainService = domainService;
  }

  @Override
  public void importAdministrativeAreas(
      AdministrativeLevel level, LandmarkMetadata metadata, Object geometry) {
    var geometriesConverted = converter.convertToLandmarksGeometries(geometry);
    geometriesConverted.forEach(
        administrativeAreaGeometry ->
            importSingleArea(level, metadata, administrativeAreaGeometry));
  }

  private void importSingleArea(
      AdministrativeLevel level,
      LandmarkMetadata metadata,
      AdministrativeAreaGeometry geometryData) {
